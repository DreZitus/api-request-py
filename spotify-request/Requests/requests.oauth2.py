...
class myOAuth2:
    ...
    def acess_token_get(self, client_id, client_secret, scope, redirect_uri, token):
        ...
        if token:
            ...
            autorization_url, state=client.autorization_url("https://accounts.spotify.com/authorize")
            print(' \n \n ppor favor visite esta URL e obtenha a URL callback: \n' + authorization_url)


            redirect_respose = input('\n \nCole a redirect URL> ')

            token = client.fetch_token("http://accounts.spotfy.com/api/token",
                                       authorization_response=redirect_respose,
